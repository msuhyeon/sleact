...
const config: webpack.Configuration = {
  ...
  output: {
    // 클라이언트 부터 해서 로더타서 만들어낸 결과물이 아웃풋에서 설정된대로 나옴
  ...
  devServer: {
    // setting한 이유 1. hot reloading 2. proxy server
    ...
    proxy: {
      // proxy에서 /api로 보내는 요청은 주소를 3095로 보내는 것 처럼 취급하겠다 뜻(3090이 아닌 3095인 것 처럼 속임) 그래서 개발자 도구의 network탭에서 확인해보면 options 요청이 안감!
      '/api/': {
...
if (isDevelopment && config.plugins) {
  ...
  //     analyzerMode: 'server', // dev에서는 analyzer가 서버를 따로 띄워서 보이고
  //     openAnalyzer: true,
  //   }),
  // );
}
if (!isDevelopment && config.plugins) {
  // 거의 영향 없지만 이게 있으면 좀 더 최적화 되는 옛날 플러그인이있어서 추가
  config.plugins.push(new webpack.HotModuleReplacementPlugin({ minimize: true }));
  config.plugins.push(
    new BundleAnalyzerPlugin({
      analyzerMode: 'static', // deploy에서는 서버를 따로 띄울 수 없기 때문에 html로 결과물을 출력해줌
    }),
  );
}
